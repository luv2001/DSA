...
int maxProfitDP(vector<int> prices, int bought , int t, int N)
{
	...
	// if (dp[N][bought][t] != -1) return dp[N][bought][t];
	...
	{	int y = min(t, prices[N - 1]);
		if (t < prices[N - 1])
		{
			...
			int buy =  prices[N - 1] - t + maxProfitDP(prices, bought - 1, prices[N - 1], N - 1);
			int not_buy = maxProfitDP(prices, bought, y, N - 1);

			// dp[N][bought][t] = max(buy, not_buy);
		...
		else
		{
			...
			// dp[N][bought][t] = maxProfitDP(prices, bought, y, N - 1);
			return maxProfitDP(prices, bought, y, N - 1);
		}
	}
}
